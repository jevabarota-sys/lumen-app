...
class SentimentAnalyzer {
  ...
  static int _countPositiveWords(String content) {
    final positiveWords = [
      ...
      'success', 'successful', 'achieve', 'achieved', 'accomplish', 'accomplished',
  ...
  static List<String> _detectEmotions(String content) {
    ...
    // Joy
    if (content.contains(RegExp(r'\b(happy|joy|excited|thrilled|delighted)\b'))) {
      emotions.add('joy');
    }
    
    // Sadness
    if (content.contains(RegExp(r'\b(sad|depressed|heartbroken|crying|tears)\b'))) {
    ...
    // Fear/Anxiety
    if (content.contains(RegExp(r'\b(afraid|scared|anxious|worried|panic|stressed)\b'))) {
    ...
    // Gratitude
    if (content.contains(RegExp(r'\b(grateful|thankful|blessed|appreciate)\b'))) {
      emotions.add('gratitude');
    }
    
    // Hope
    if (content.contains(RegExp(r'\b(hope|hopeful|optimistic|encouraged|inspired)\b'))) {
      emotions.add('hope');
    }
    
    // Loneliness
    if (content.contains(RegExp(r'\b(lonely|alone|isolated|abandoned|disconnected)\b'))) {
      emotions.add('loneliness');
    }
    
    // Confusion
    if (content.contains(RegExp(r'\b(confused|uncertain|lost|stuck|doubt)\b'))) {
  ...
  static String _detectIntensity(String content) {
    ...
    final highIntensityWords = [
      ...
      'utterly', 'so much', 'really', 'truly', 'deeply', 'profoundly',
    ];
  ...
  static Map<String, dynamic> analyzeTrends(List<Map<String, dynamic>> entries) {
    if (entries.isEmpty) {
      return {
        'averageScore': 0.0,
        'trend': 'stable',
  ...
  static String generateInsight(Map<String, dynamic> analysis) {
    ...
    if (sentiment == 'positive') {
      ...
      if (intensity == 'high') {
        insight += 'The strong positive energy in your words suggests you\'re experiencing significant joy or satisfaction. ';
      }
    } else if (sentiment == 'negative') {
      insight += 'Your entry shows you\'re working through some challenging emotions. ';
      if (intensity == 'high') {
        insight += 'The intensity of your feelings suggests this is weighing heavily on you. Remember, it\'s okay to feel this way. ';
      }
    } else {
    ...
    if (emotions.contains('anxiety')) {
      insight += 'I notice anxiety present in your words. Consider grounding practices like deep breathing or connecting with nature. ';
    }
    if (emotions.contains('gratitude')) {
      insight += 'Your gratitude is beautiful and helps cultivate positive energy. ';
    }
    if (emotions.contains('loneliness')) {
      insight += 'Feelings of loneliness are valid. Consider reaching out to someone you trust or engaging in activities that bring you connection. ';
    }
    if (emotions.contains('hope')) {
      insight += 'Your hope is a powerful force for positive change. ';
    }
    if (emotions.contains('confusion')) {
      insight += 'When feeling confused, journaling more can help clarify your thoughts and feelings. ';
    }
    
    // Score-based encouragement
    if (score < -0.5) {
      insight += 'Remember, difficult times are temporary. You have the strength to navigate this.';
    } else if (score > 0.5) {
  ...
  static String generateTrendInsight(Map<String, dynamic> trendAnalysis) {
    ...
    if (trend == 'improving') {
      insight += 'Your emotional well-being shows a positive upward trend! This suggests your growth practices are working. ';
    } else if (trend == 'declining') {
      insight += 'I notice your mood has been trending downward. This might be a good time to focus on self-care and reach out for support if needed. ';
    } else {
      insight += 'Your emotional state has been relatively stable. ';
    }
    
    if (averageScore > 0.3) {
      insight += 'Overall, you\'ve been experiencing more positive than negative emotions. ';
    } else if (averageScore < -0.3) {
      insight += 'You\'ve been working through some challenging emotions. Be gentle with yourself. ';
    }
    
    if (dominantEmotion != 'neutral') {
      insight += 'The emotion that appears most frequently is $dominantEmotion. ';
      
      if (dominantEmotion == 'anxiety') {
        insight += 'Consider incorporating more grounding and calming practices into your routine.';
        ...
        insight += 'It\'s important to honor these feelings while also nurturing hope.';
      }
    }
